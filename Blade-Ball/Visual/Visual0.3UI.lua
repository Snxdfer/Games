...
function visual: init(name: string, is_draggable: boolean, parent)
    ...
    ImageLabel.Image = "rbxassetid://17441779136"
    ...
    task.defer(function()
        user_input.InputEnded:Connect(function(input, is_event)
            if not is_event and (input.KeyCode == Enum.KeyCode.RightAlt or input.KeyCode == Enum.KeyCode.Insert)  then
...
function visual: create_section(name: string, imageID: number)
    task.wait(0.5)
...
function visual: create_toggle(name: string, section_name: string, callback)
    task.wait(0.15)
