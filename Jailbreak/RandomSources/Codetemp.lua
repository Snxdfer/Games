...
local ServerHop = LPH_NO_VIRTUALIZE(function()
    if queued == false then
        ...
        if getgenv().WebhookUrl ~= "" and getgenv().LogWebhook and not SentWebhookServerhop then
            LogStatus(1, nil)
            SentWebhookServerhop = true
        end
        ...
        queue = queue .. " loadstring(game:HttpGet(getgenv().ScriptUrl))()"
    ...
    while true do
        pcall(function()
            local ServersEndpoint = "https://games.roblox.com/v1/games/" .. game.PlaceId .. "/servers/Public?sortOrder=Asc&limit=100"
            local Server, Next = nil, nil

            local function ListServers(cursor)
                local Raw = game:HttpGet(ServersEndpoint .. (cursor and "&cursor="..cursor or ""))
		...
		local function Advertise()
			...
			local Messages = {
				"BEST JAILBREAK MONEY MAKER!!",
				"GOOD FOR HYPERS!!",
				"TRY IT, YOUR DAD COMES BACK!!",
				"JOIN THE COOL KIDS!!",
